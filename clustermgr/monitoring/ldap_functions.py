...
class LdapOLC(object):
    ...
    def getReplicationStatus(self):
        """Returns replication status based on contextCSN

        Returns:
            list of epoch contextCSN times
        """
        ret_list = []
        self.conn.search(search_base='o=gluu', search_scope=BASE, search_filter='(objectclass=*)', attributes=['+'])
        for ce in self.conn.response[0]['attributes']['contextCSN']:
            t=time.strptime(ce[:14], "%Y%m%d%H%M%S")
            tt = time.mktime(t)
            ret_list.append(tt)
        return ret_list
